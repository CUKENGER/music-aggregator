...
function isDeezerSearchResponse(data: unknown): data is DeezerSearchResponse {
  return (
    typeof data === 'object' &&
    data !== null &&
    Array.isArray((data as DeezerSearchResponse).data)
  );
}
...
export class DeezerProvider implements IMusicProvider {
  ...
  async search(query: string, limit = 5): Promise<TrackMeta[]> {
    ...
    try {
      ...
      await this.redis.set(cacheKey, JSON.stringify(tracks), 'EX', 3600);
      ...
      this.logger.error(`Deezer search error`);
      return [];
    }
  }
  ...
  getStream(id: string): Promise<string> {
    ...
    return Promise.resolve(`https://www.deezer.com/track/${id}`);
  }
}
